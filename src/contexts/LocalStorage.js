...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(EDDASWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(EDDASWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
