...
const SORT_FIELD = {
  VALUE: 'VALUE',
  EDDASWAP_RETURN: 'EDDASWAP_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.EDDASWAP_RETURN) {
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.EDDASWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.EDDASWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.EDDASWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
